...
- Floating toolbar (bottom center, drag via the **drag handle**):
  ...
  - Pen / Eraser, **3 color swatches** (defaults: red, blue, yellow), stroke **size slider** (each tool remembers its own size; pen max 24px, eraser max 128px); in Draw mode, mouse wheel also changes size for the active tool
...
- Shortcuts (while Draw mode has keyboard focus):
  ...
  - `P` pen, `E` eraser (stylus button 2 click toggles; hold + left/right cycles colors; button 3 click cycles presets / drag adjusts size)
